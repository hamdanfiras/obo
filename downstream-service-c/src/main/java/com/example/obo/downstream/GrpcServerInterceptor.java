...
public class GrpcServerInterceptor implements ServerInterceptor {
    ...
    public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
        ...
        try {
            ...
            if (!"downstream-service-c".equals(audience)) {
                call.close(Status.PERMISSION_DENIED.withDescription("Invalid audience: " + audience), new Metadata());
                return new ServerCall.Listener<ReqT>() {};
            }

            // Validate scope
            String scope = jwt.getClaimAsString("scope");
            if (scope == null || !scope.contains("payments.finalize")) {
                call.close(Status.PERMISSION_DENIED.withDescription("Missing required scope"), new Metadata());
                return new ServerCall.Listener<ReqT>() {};
            }
            ...
            return Contexts.interceptCall(Context.current().withValue(AuthenticatingServerInterceptor.SECURITY_CONTEXT_KEY, securityContext), call, headers, next);
        } catch (Exception e) {
